...
- The nav renders in three tiers, each with its **own CSS class and its own active-state updater function** — deliberately not one shared class/loop: the primary `.tabs` row (`.dest-btn` — Find Customers, Size a Market) via `updateDestActive(tab)`; the secondary `#findCustomersModeRow` (`.mode-btn` — By Name/By Code/By Location, shown only when `currentTab` is in `FIND_CUSTOMER_MODES`) via `updateModeActive(tab)`; and `.utility-strip` (`.tab-utility` — Look up a code, Verify a provider) via `updateUtilityActive(tab)`. `switchTab()` calls all three. **This structure exists because two earlier bugs both came from sharing one `.tab` class across all three tiers** — an `.active` rule for one tier left a CSS property (background/color) unset, and a same-specificity rule from a *different* tier leaked through per CSS's per-property (not per-rule) cascade. Separate classes make that class of bug structurally impossible: no rule can apply to more than one tier's elements. When adding a 4th nav tier, give it its own class and its own updater — do not add it to `.dest-btn`/`.mode-btn`/`.tab-utility` or reintroduce a shared class.
...
- `onTabKeydown(e)` (arrow-key cycling) scopes to `e.currentTarget` so the primary row and the mode row each cycle independently, and queries by `[role="tab"]` rather than a specific class — tier-agnostic, so it needs no change if a tier's class is ever renamed again.
