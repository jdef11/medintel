...
├── medintel-core.test.js         # Vitest suite for medintel-core.js (272 tests)
...
- **Find Customers** (`data-tab="provider"`) — CMS Medicare | First name (optional, individuals only) + last name **or** organization name (separate fields), NPI, specialty (searchable dropdown, see below), state, city — fill in whichever apply | `Rndrng_Prvdr_Last_Org_Name` CONTAINS + entity-type disambiguation (`Rndrng_Prvdr_Ent_Cd`): last/first name excludes `'O'` client-side, organization requires `'O'`, mutually exclusive and validated up front; `Rndrng_Prvdr_First_Name` CONTAINS (lenient) for first name; `Rndrng_Prvdr_Type` CONTAINS for specialty; `Rndrng_Prvdr_State_Abrvtn`/`Rndrng_Prvdr_City` for state/city.
  ...
  **Group by practice** and **Group by hospital** are two mutually-exclusive rollup modes, each with a pair of checkboxes — one in the sidebar (`groupByPracticeToggle`/`groupByHospitalToggle`, settable *before* a search runs) and one in the post-results filter bar (`...ToggleInline`, rendered fresh by `renderResults()` each time). `setGroupMode('none'|'practice'|'hospital')` is the single entry point for all four checkboxes: it sets `groupByPractice`/`groupByHospital`, keeps all four checkboxes in sync, and either kicks off resolution immediately (a search has already rendered) or just remembers the preference for `executeSearch()`'s own pre-search read (see below). Checking one always unchecks the other — a billing group and a hospital are different facts, and rendering both rollups at once isn't a coherent view. `getFilteredSorted()`/`activeResultsBase()` pick `practiceGroupedResults`, `hospitalGroupedResults`, or `allGroupedResults` as their base depending on which mode is active, so the existing volume filter/sort/pagination pipeline needs no duplication per mode. `exportCSV()` and `exportSharePointCSV()` each branch on `groupByPractice`/`groupByHospital` to emit group-level rows instead of per-provider ones.

  Both modes resolve over the **entire** result set (not just the current page, since a group's members can land on different pages), and both are genuinely slow — the underlying `data.cms.gov/provider-data` host has a flat ~13-15s-per-request cost regardless of batch size (see External APIs below), so resolution is batched (`PRACTICE_BATCH_SIZE` NPIs per request, `PRACTICE_BATCH_CONCURRENCY` concurrent batches via `runWithConcurrencyCap`) rather than parallelized per-NPI. If the sidebar checkbox was checked before searching, `executeSearch()` resolves affiliations itself — under the same spinner the user is already watching — before the first render, rather than requiring a second toggle click after results appear. Toggling a mode off and back on re-renders instantly from `practiceGroupedResults`/`hospitalGroupedResults` with no re-fetch. A multi-member group card skips the true-unique-beneficiary fetch (`patchProviderBenes`) and hides the per-provider trend-chart affordance — neither is obtainable/practical at the group level (see `providerCardHtml`/`practiceCardHtml`/`hospitalCardHtml`); a solo group (`memberCount === 1`) renders identically to the ordinary per-provider card.

  - **Group by practice** rolls providers up by `org_pac_id` from the "Doctors and Clinicians National Downloadable File" (CMS Provider Data Catalog, dataset `mj5m-pzi6` — a different host/API than the `data-api` claims datasets), sourced from PECOS Medicare enrollment records — a real billing-group crosswalk, not an address-matching guess. `fetchPracticeAffiliationsBatch(npis)` returns every enrollment record CMS has for a batch of NPIs — **a provider can have more than one** (verified live: NPI `1548269731` has three, two of them at the identical address) — and `resolveOneAffiliation()` (medintel-core.js, pure) picks the ONE whose (city, state, zip) uniquely matches the provider's own claims-derived address; if none match uniquely, it falls back to the largest group by membership and marks the pick `disambiguated: true` rather than presenting a guess as certain (surfaced as a "≈" marker on the practice card). `groupProvidersByPractice()` (medintel-core.js, pure) then merges procedures by `(code, place)` — not concatenating them — so a code two members both bill doesn't inflate the practice's `computeComplexityScore` breadth purely from headcount.
  - **Group by hospital** rolls providers up by real hospital facility affiliation instead of a billing group — a genuinely different fact from `org_pac_id`. It queries a **different** dataset on the same host, "Facility Affiliation Data" (`27ea-46a8`, filtered client-side to `facility_type === 'Hospital'` — the file also covers home health/rehab/hospice/SNF/dialysis, which this feature ignores) via `fetchFacilityAffiliationsBatch(npis)`/`resolveHospitalAffiliations()`, then resolves each unique CCN it turns up to a real name/city/state/star-rating via a second, much smaller lookup against "Hospital General Information" (`xubh-q36u`) via `fetchHospitalInfoBatch(ccns)`. Unlike practice grouping, this relationship is **verified live to be genuinely one-to-many in both directions with no disambiguation** — a provider can be affiliated with more than one real hospital — so `groupProvidersByHospital()` (medintel-core.js, pure) deliberately keeps ALL of them: a provider affiliated with 2 hospitals appears on BOTH cards. That means summing totals across every hospital card overstates the true provider-level total (each card's own numbers are still accurate on their own) — surfaced via a year-note in `renderResults()`. `providerDataApiBase(datasetId)` generalizes the shared host/path shape across all three provider-data datasets (`mj5m-pzi6`, `27ea-46a8`, `xubh-q36u`) used by these two features.
...
### Provider Data Catalog (a THIRD host, live but not `data-api`) — powers Group by practice / Group by hospital
...
`providerDataApiBase(datasetId)` builds this URL for **three separate datasets** on the same host/path shape (`data.cms.gov/provider-data/api/1/datastore/query/...`, not `data.cms.gov/data-api/v1/dataset/...` — a **third distinct host/path shape** from everything else in this app):

- **`mj5m-pzi6`** — "Doctors and Clinicians National Downloadable File," sourced from PECOS Medicare enrollment records. Gives `org_pac_id` (billing-group crosswalk) — powers **Group by practice**. Fields used: `npi`, `org_pac_id`, `facility_name`, `num_org_mem`, `citytown`, `state`, `zip_code`.
...
| `renderResults()` | Renders Medicare provider cards to DOM. Branches per card into `providerCardHtml()` (per-provider, or a solo group), `practiceCardHtml()` (a real multi-member practice), or `hospitalCardHtml()` (a real multi-member hospital rollup) depending on `groupByPractice`/`groupByHospital` |
...
| `resolveOneAffiliation(candidates, provider)` / `groupProvidersByPractice(providers, affiliationByNpi)` | (pure, in medintel-core.js) Practice-rollup logic for "Group by practice" — see Search Modes above |
| `groupProvidersByHospital(providers, hospitalsByNpi, hospitalInfoByCcn)` | (pure, in medintel-core.js) Hospital-rollup logic for "Group by hospital" — unlike `groupProvidersByPractice`, deliberately allows a provider to appear in more than one output group (real one-to-many hospital affiliation) — see Search Modes above |
| `setGroupMode('none'\|'practice'\|'hospital')` | Single entry point for all four "Group by..." checkboxes (sidebar + inline, practice + hospital) — sets `groupByPractice`/`groupByHospital`, syncs every checkbox, and either resolves immediately (results already rendered) or just remembers the preference for `executeSearch()`'s pre-search read |
| `fetchPracticeAffiliationsBatch(npis)` / `resolvePracticeAffiliations(providers, progress)` / `applyPracticeGrouping(checked)` | CMS Provider Data Catalog (`mj5m-pzi6`) crosswalk lookup, batch resolution, and the toggle handler — see Search Modes above. `resolvePracticeAffiliations` chunks uncached providers into `PRACTICE_BATCH_SIZE` (50)-NPI batches (one `in`-operator query per batch, not one per NPI) run at `PRACTICE_BATCH_CONCURRENCY` (3) concurrent batches via `runWithConcurrencyCap`, each with a `PRACTICE_FETCH_TIMEOUT_MS` (60000) override passed through `corsFetch`'s `opts.directTimeoutMs`/`proxyTimeoutMs` — the endpoint's flat ~15s-per-request cost makes batching, not added concurrency, the actual fix for slow resolution on large result sets |
| `fetchFacilityAffiliationsBatch(npis)` / `fetchHospitalInfoBatch(ccns)` / `resolveHospitalAffiliations(providers, progress)` / `applyHospitalGrouping(checked)` | Mirrors the practice-affiliation stack one level up: batches NPIs against `27ea-46a8` (same batch/concurrency/timeout constants), then batches the small set of unique CCNs that turned up against `xubh-q36u` for names — see Search Modes above |
| `resolvePracticeGroupsFor(providers, onProgress)` / `resolveHospitalGroupsFor(providers, onProgress)` | Shared core of the toggle handler and `executeSearch()`'s pre-search path for each mode — resolve affiliations then roll up + score in one call, so the two call sites can't drift |
| `activeResultsBase()` | The raw (unfiltered-by-volume) array behind whichever grouping mode is active (`hospitalGroupedResults`/`practiceGroupedResults`/`allGroupedResults`) — used by `getFilteredSorted()` and by the "N below volume threshold" count in `renderResults()`, which must compare against the active mode's own card count (a rollup can have fewer OR — for hospital grouping — more cards than raw providers) |
| `exportCSV()` | Downloads grouped provider data as CSV — practice-mode or hospital-mode rows depending on `groupByPractice`/`groupByHospital` |
...
Pure logic lives in `medintel-core.js` and is unit-tested with Vitest (`npm test` → `medintel-core.test.js`, 272 tests). The GitHub Pages deploy runs the suite before publishing. Additionally:
